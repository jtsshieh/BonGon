exports.run = (bot, msg, args, prefix) => {
    ...
    if (!args[0]) {
        let embed = bot.buildPlayer(`The current volume for this queue is ${server.dispatcher.volume}%. Use ${prefix}volume to change it`, [], [], [], server.nowPlaying.thumbnail, [server.nowPlaying.title, server.nowPlaying.thumbnail]);
    ...
    let volume = Number(args[1]) / 100;
    if (server.dispatcher) {
        server.dispatcher.setVolume(volume);
        msg.channel.createMessage(`Volume set: ${volume}%`);
    }
};
